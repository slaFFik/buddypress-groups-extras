...
jQuery( document ).ready( function( $ ) {
	...
	jQuery( '#bp-gtm-admin-table .bpge_allgroups' ).change( function() {
		...
		if ( isChecked ) {
			jQuery( '#bp-gtm-admin-table .bpge_groups' ).prop( 'checked', true );
			jQuery( '#bp-gtm-admin-table .bpge_allgroups' ).prop( 'checked', true );
		}
		else {
			jQuery( '#bp-gtm-admin-table .bpge_groups' ).prop( 'checked', false );
			jQuery( '#bp-gtm-admin-table .bpge_allgroups' ).prop( 'checked', false );
		}
	} );
	...
	jQuery( '#bpge-admin .set_apply' ).click( function( e ) {
		...
		new Messi( bpge.apply_set, {
			title: set_name,
			buttons: [ { id: 0, label: bpge.yes, val: 'Y' },
	...
	jQuery( '#extra-field-vars a.remove_it' ).on( 'click', function( e ) {
		e.preventDefault();
	...
	jQuery( '#extra-field-vars a#add_new' ).on( 'click', function( e ) {
		e.preventDefault();
		options_count += 1;
